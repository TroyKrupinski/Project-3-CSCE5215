...
def calculate_prob_car_wont_start_given_battery_age():
    # Prior probabilities
    ...
    # Calculate P(car_won't_start_y|Battery_age_y)
    total_prob = 0
    evidence = 'y'  # Battery_age = y

    # Generate all possible combinations of other variables
    states = ['y', 'n']
    variables = ['alternator_broken', 'fanbelt_broken', 'battery_dead', 'no_charging', 'battery_flat', 'no_oil']
    
    for combo in itertools.product(states, repeat=len(variables)):
        ab, fb, bd, nc, bf, no = combo
        
        # Calculate probability for this combination
        p = (P_alternator_broken[ab] *
             P_fanbelt_broken[fb] *
             P_battery_dead[(evidence, bd)] *  # Using Battery_age = y as evidence
             P_no_charging[(ab, fb, nc)] *
...
result = calculate_prob_car_wont_start_given_battery_age()
print(f"P(+car_won't_start|+Battery_age) = {result:.4f}")